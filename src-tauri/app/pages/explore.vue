...
function clickHandler() {
  ...
  const API_BASE_URL = 'https://devtest.doshare.me'; // Adjust this to your backend URL
        const userId = authdata.data.user.id; // Sample user ID
var myfiles = null;
const {data,error}= await useFetch(`${API_BASE_URL}/search?q=&user_id=${userId}`);
if(data.value!=null){
  myfiles = [];
  ...
  for (let i = 0; i < data.value.length; i++) {
    ...
    console.log(myfiles);
        ...
        function searchFiles() {
            ...
            axios.get(`${API_BASE_URL}/search?q=${query}&user_id=${userId}`)
                .then(response => {
...
<div class="suse-400 text-center content-center justify-items-center">
  <!-- If Empty -->

        <h2 class="text-2xl m-2">Let's fill up this folder</h2>
        <div class="flex flex-grow flex-col">
    <img class="content-center self-center justify-center items-center object-center" src="https://pub-a1d8320a13a348a8abae7ee559fd2c49.r2.dev/ruby-box-filled-with-stuff.png" alt="">
        </div>
